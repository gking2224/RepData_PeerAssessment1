...
```
Mean number of steps taken per day is `r mean`  
Median number of steps taken per day is `r median`
...
```
The 5 minute interval with most steps on average is 08:35-08:40, with `r most.active.interval["int.avg"]` steps on average.

## Imputing missing values
There are `r sum(is.na(csv$steps))` missing values in the dataset. These will be substituted in a new variable `csv2` with the median value for the relevant interval across all dates in the dataset.
...
This new dataset with imputed values can be compared with the original data:

While there are `r sum(is.na(csv$steps))` observations with no values in the original dataset (`csv`)...
```{r comp.na.obs1}
print(sum(is.na(csv$steps)))
```
...this is now `r sum(is.na(csv2$steps))` in the imputed dataset(`csv2`), as you would expect:
```{r comp.na.obs2}
print(sum(is.na(csv2$steps)))
```

In the daily summary dataset (`ds`), there were `r nrow(ds[ds$steps==0 | is.na(ds$steps),])` days with zero or NA steps taken  ...
```{r comp.of.lazy.days1}
print(nrow(ds[ds$steps==0 | is.na(ds$steps),]))
```
...whereas after imputing the missing values (`ds2`), there are `r nrow(ds2[ds2$steps==0 | is.na(ds2$steps),])` days with zero steps taken:
...
```
The total number of steps in the imputed dataset is `r sum2`, which is `r sum2 - sum(ds$steps, na.rm=TRUE)` greater than in the original dataset.
...
```
Mean number of steps taken per day is `r mean2` when including imputed data, the same as the mean of the original dataset. This will be on account of the missing values being the means themselves.

Median number of steps taken per day is `r median2` when including imputed data, which is only `r median2 - median` more than the median of the non-imputed dataset.
